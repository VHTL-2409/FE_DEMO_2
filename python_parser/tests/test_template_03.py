...
D. x = 4"""
    ...
    def test_parse_options_inline_without_spaces_after_period(self):
        """pdf_mau_3 mới: marker dính kiểu 'A. foo.B. bar.C. baz.D. qux' vẫn phải tách được."""
        text = "A. y=foo.B. y=bar.C. y=baz.D. y=qux"
        parser = Template03MathAnswerGridParser.__new__(Template03MathAnswerGridParser)
        options = parser._parse_options(text)

        assert set(options.keys()) == {"A", "B", "C", "D"}
        assert "foo" in options["A"]
        assert "bar" in options["B"]
        assert "baz" in options["C"]
        assert "qux" in options["D"]
...
class TestPdfMau3StemBoundary:
    ...
    def test_mapped_private_use_glyphs_still_render_as_image_for_math_mcq(self):
        """Math MCQ cần giữ layout bằng ảnh ngay cả khi glyph đã map sạch."""
        block = ParsedBlock(
            raw_text=(
        ...
        parser._answer_key = {}
        parser._solutions = {}
        parser._question_regions_by_number = {}
...
class TestTemplate03Bbox:
    def test_bbox_uses_trimmed_token_bounds_instead_of_full_page_width(self):
        ...
        parser._question_regions_by_number = {
            3: QuestionRegion(
                ...
                y1=300.0,
                raw_lines=[
                    "Câu 3: Đồ thị như hình vẽ là của hàm số nào?",
                    "1",
                    "A. y = x^3",
                ],
                raw_text="",
                raw_tokens_by_line=[
                    [Word(text="Câu", x0=36.0, y0=508.0, x1=62.0, y1=521.0)],
                    [Word(text="1", x0=303.0, y0=731.0, x1=309.0, y1=742.0)],
                    [Word(text="A.", x0=54.0, y0=293.0, x1=68.0, y1=308.0), Word(text="x^3", x0=90.0, y0=293.0, x1=120.0, y1=308.0)],
                ],
            )
        }

        bbox = parser._bbox_for_question(3)

        assert bbox == (24.0, 285.0, 132.0, 529.0)

    def test_bbox_stops_before_next_question_on_same_page(self):
        from app.utils.math_text_engine import QuestionRegion, Word
        ...
        parser._question_regions_by_number = {
            3: QuestionRegion(
                ...
                raw_tokens_by_line=[
                    [Word(text="Câu", x0=36.0, y0=508.0, x1=62.0, y1=521.0)],
                    [Word(text="A.", x0=54.0, y0=533.0, x1=68.0, y1=548.0), Word(text="x^3", x0=90.0, y0=533.0, x1=120.0, y1=548.0)],
                ],
            ),
            4: QuestionRegion(
                number=4,
                ...
                y0=560.0,
                y1=610.0,
                raw_lines=["Câu 4: Câu kế tiếp"],
                raw_text="",
                raw_tokens_by_line=[
                    [Word(text="Câu", x0=36.0, y0=560.0, x1=62.0, y1=573.0)],
                ],
            ),
        }
        ...
        assert bbox == (24.0, 500.0, 132.0, 554.0)
...
class TestAnswerKeyExtraction:
    ...
    def test_extract_from_solution_section(self):
        """Solution-only text must not be treated as answer key."""
        text = """Câu 1: Lời giải
→ Chọn B.
