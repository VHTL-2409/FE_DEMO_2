...
class TestDetectSections:
    ...
    def test_part3_mcq(self):
        # Keyword in title must override roman parity.
        text = "PHẦN I: Trắc nghiệm\n1. Câu hỏi\nPHẦN III: Tự luận\n2. Bài toán"
        sections = detect_sections(text)
        kinds = [s.kind for s in sections]
        assert len(kinds) >= 2
    ...
    def test_normalize_section_key_removes_accents_and_spacing(self):
        assert normalize_section_key("Phần III: Tự luận") == "phaniiituluan"

    def test_detects_huong_dan_giai_as_solution(self):
        assert detect_section_header("Hướng dẫn giải chi tiết") == SectionKind.SOLUTION

    def test_detects_dap_an_as_answer_key(self):
        assert detect_section_header("Phần đáp án") == SectionKind.ANSWER_KEY
