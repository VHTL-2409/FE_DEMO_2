...
class AnswerExtractor:
    ...
    # NOTE: Intentionally excludes solution-only headers because auto grading
    # should not read answers from the explanation section.
    ANSWER_MARKERS = [
        "BẢNG ĐÁP ÁN",
        "ĐÁP ÁN",
    ...
    def __init__(
        ...
        *,
        ...
        self.allow_solution_fallback = allow_solution_fallback
    ...
    def extract(self) -> list[AnswerEntry]:
        ...
        Strategy:
          ...
          4. Optionally use solution-style fallback when explicitly enabled
        """
        ...
        # ── Step 4: Optional solution-style fallback ───────────────────────────
        if self.allow_solution_fallback and len(answers) < 40:
    ...
    def get_as_dict(self) -> dict[int, str]:
        ...
        return {e.number: e.answer for e in self.extract() if e.number > 0}
