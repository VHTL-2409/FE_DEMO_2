...
def fix_pdf_ambiguous_exponents_to_unicode(text: str) -> str:
    ...
    Heuristic bảo thủ:
    - biến một ký tự thường gặp ở dạng lowercase + một chữ số 2–9
    - biểu thức đóng ngoặc như (2x+1)2
    - chỉ đổi khi phía sau là toán tử / dấu câu / kết dòng

    Tránh khớp mã câu kiểu \"Câu 2\" hoặc set-notation kiểu \"S3 = -\".
    """
    if not text:
        return text
    sup_map = {
    ...
    result = re.sub(
        rf"(?<![A-Za-z0-9À-ỹà-ỹ])([a-z])([2-9]){trailing_boundary}",
        repl,
        text,
    )
    result = re.sub(
        rf"([\)\]])([2-9]){trailing_boundary}",
        repl,
        result,
    )
    return result
...
def has_unmapped_pdf_private_use_math_glyphs(text: str) -> bool:
    """True khi vẫn còn glyph Private Use sau bước map các ký hiệu đã biết."""
    if not text:
        return False
    return bool(_PRIVATE_USE_CHAR_RE.search(map_pdf_private_use_math_glyphs(text)))


def sanitize_pdf_private_use_math_glyphs(text: str) -> str:
    """
    Map các glyph Private Use đã biết sang Unicode rồi loại bỏ phần còn lại.
    Dùng cho text fallback / latex conversion để tránh rò PUA ra FE.
    """
    if not text:
        return text
    mapped = map_pdf_private_use_math_glyphs(text)
    mapped = _PRIVATE_USE_CHAR_RE.sub(" ", mapped)
    mapped = re.sub(r"[ \t]{2,}", " ", mapped)
    mapped = re.sub(r" *\n *", "\n", mapped)
    return mapped.strip()
...
def normalize_mcq_stem_display(text: str) -> str:
    ...
    text = text.replace("\ufeff", "")
    text = text.replace("\ufffd", "")
    text = sanitize_pdf_private_use_math_glyphs(text)
...
def normalize_math_text(text: str, *, preserve_private_use: bool = False) -> str:
    ...
    if not preserve_private_use:
        text = _PRIVATE_USE_CHAR_RE.sub("", text)
