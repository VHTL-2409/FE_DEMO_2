...
class Template03MathAnswerGridParser(BaseParser):
    ...
    def __init__(self, pdf_path: str, session_id: str | None = None):
        ...
        self._question_regions_by_number: dict[int, object] = {}
    ...
    def _parse_impl(self) -> tuple[list[ParsedQuestion], ExamMeta]:
        ...
        self._question_regions_by_number = self._extract_question_regions_by_number()
    ...
    def _parse_question_block(self, block: ParsedBlock) -> Optional[ParsedQuestion]:
        ...
        display_options = {
            k: sanitize_pdf_private_use_math_glyphs(v)
        ...
        has_option_markers = any(
            re.search(rf"(?:^|\s){letter}\s*[\.)．:]", clean)
            for letter in "ABCD"
        )
        has_private_use = has_unmapped_pdf_private_use_math_glyphs(raw)
        malformed_options = len(valid_options) < 4 and has_option_markers
        high_noise = formula_noise > 0.18 or has_private_use or malformed_options

        nonempty_opts = sum(1 for v in valid_options.values() if v.strip())
        stem_latex = convert_to_latex(stem, mode="auto")
        option_text_for_type = "\n".join(valid_options.values())
        content_type = classify_content_type(
            "\n".join(part for part in [stem, option_text_for_type] if part.strip()),
            "\n".join(
                part for part in [
                    stem_latex,
        ...
        force_image_render = nonempty_opts >= 2 and content_type in {"math", "mixed"}

        # Math answer-grid questions should prefer image rendering so the full
        # visual layout remains intact for review and answer selection.
        render_mode = RenderMode.IMAGE if (high_noise or force_image_render) else RenderMode.LATEX
        q_type = (
            QuestionType.MULTIPLE_CHOICE
            if nonempty_opts >= 2 or has_option_markers
            else QuestionType.ESSAY
        )

        if high_noise:
            if formula_noise > 0.18:
                issues = [f"High formula noise ({formula_noise:.1%}), rendered as image."]
            else:
                issues = ["Rendered as image to preserve formula fidelity."]
            if has_private_use:
            ...
            issues = []
        if force_image_render and "Math question rendered as image to preserve question layout." not in issues:
        ...
        q = self.build_question(
            ...
            bbox=self._bbox_for_question(num) if (high_noise or force_image_render) else None,
            issues=issues,
            display_text=stem,
        )
        if not high_noise:
            try:
                q.latexContent = stem_latex
                q.contentType = content_type
                if valid_options:
                    q.latexOptions = {
    ...
    def _collect_option_markers(self, text: str) -> list[re.Match]:
        ...
        for m in re.finditer(
            ...
            found.append(m)
        # Compact marker: "A.y=..." or " B.x..."
        for m in re.finditer(
            r"(?<![A-Za-zÀ-ỹà-ỹ0-9])([A-D])\s*[\.)．:]\s*(?=\S)", text, re.MULTILINE
        ):
            found.append(m)
    ...
    def _calculate_formula_noise(self, text: str) -> float:
        ...
        noise += len(re.findall(r"[\uf000-\uf8ff]", text)) * 3
        return noise / max(len(text), 1)

    def _extract_question_regions_by_number(self) -> dict[int, object]:
        try:
        ...
        for region in extraction.questions:
            if region.number <= 0:
                continue
            if region.number in regions:
                continue
            regions[region.number] = region
        return regions

    def _bbox_for_question(self, question_num: int) -> Optional[tuple[float, float, float, float]]:
        region_map = getattr(self, "_question_regions_by_number", None) or {}
        region = region_map.get(question_num)
        if region is None:
            return None
        try:
            page_width, page_height = self.reader.get_page_size(region.page)
        except Exception:
            return None
        raw_x0 = None
        raw_y0 = None
        raw_x1 = None
        raw_y1 = None
        for idx, token_line in enumerate(getattr(region, "raw_tokens_by_line", []) or []):
            if not token_line:
                continue
            line_text = ""
            raw_lines = getattr(region, "raw_lines", []) or []
            if idx < len(raw_lines):
                line_text = str(raw_lines[idx]).strip()
            if re.fullmatch(r"\d+", line_text):
                continue
            line_x0 = min(tok.x0 for tok in token_line)
            line_y0 = min(tok.y0 for tok in token_line)
            line_x1 = max(tok.x1 for tok in token_line)
            line_y1 = max(tok.y1 for tok in token_line)
            raw_x0 = line_x0 if raw_x0 is None else min(raw_x0, line_x0)
            raw_y0 = line_y0 if raw_y0 is None else min(raw_y0, line_y0)
            raw_x1 = line_x1 if raw_x1 is None else max(raw_x1, line_x1)
            raw_y1 = line_y1 if raw_y1 is None else max(raw_y1, line_y1)

        if raw_x0 is None or raw_y0 is None or raw_x1 is None or raw_y1 is None:
            raw_y0 = min(region.y0, region.y1)
            raw_y1 = max(region.y0, region.y1)
            raw_x0 = 24.0
            raw_x1 = page_width - 24.0

        y0 = max(0.0, raw_y0 - 8.0)
        next_same_page_starts = sorted(
            min(other.y0, other.y1)
            for num, other in region_map.items()
            if num != question_num
            and getattr(other, "page", None) == region.page
            and min(other.y0, other.y1) > raw_y1
        )
        y1 = min(page_height, raw_y1 + 8.0)
        if next_same_page_starts:
            y1 = min(y1, max(raw_y1 + 2.0, next_same_page_starts[0] - 6.0))
        if y1 <= y0:
            return None
        x0 = max(0.0, raw_x0 - 12.0)
        x1 = min(page_width, raw_x1 + 12.0)
        if x1 <= x0:
            return None
        return (x0, y0, x1, y1)
