        ...
        <div class="qemodal" :class="`qemodal--${size}`">
          ...
          <div class="qemodal__body">
            ...
            <div class="qemodal__field">
              ...
              />
              <div v-if="localContent.trim()" class="qemodal__preview">
                <div class="qemodal__preview-label">Xem trước</div>
                <div class="qemodal__preview-surface">
                  <MathDisplay
            ...
            <div v-if="!isEssay" class="qemodal__field">
              ...
              <div class="qemodal__options-grid">
                <div v-for="opt in optionKeys" :key="opt" class="qemodal__opt">
                  ...
                  />
                  <div v-if="localOptions[opt].trim()" class="qemodal__opt-preview">
                    <MathDisplay
                      :content="localOptions[opt]"
                      :latex-content="null"
                      source-preference="latex-first"
                    />
                  </div>
...
const parseQuestionOptions = (question) => {
  ...
  if (Array.isArray(parsed)) {
    return parsed.map((item, index) => ({
      id: String(item?.id ?? optionKeys[index] ?? index + 1).toUpperCase(),
      text: String(item?.text ?? item?.value ?? item?.content ?? '').trim()
    }))
  }
...
const optionTextFromQuestion = (question, optionId) => {
  const normalizedId = String(optionId).toUpperCase()
  const parsedOptions = parseQuestionOptions(question)
  const found = parsedOptions.find((opt) => opt.id === normalizedId)
  if (found) return found.text
  return String(question?.[`option${normalizedId}`] ?? '').trim()
}
...
const currentOptionMap = () => optionKeys.reduce((acc, key) => {
  acc[key] = localOptions[key].trim()
  return acc
}, {})

const selectedAnswersMissingText = () => {
  const optionMap = currentOptionMap()
  return localCorrectAnswers.value.some((key) => !optionMap[key])
}

const originalOptionMap = () => optionKeys.reduce((acc, key) => {
  acc[key] = optionTextFromQuestion(props.question, key)
  return acc
}, {})

const haveOptionsChanged = () => {
  const current = currentOptionMap()
  const original = originalOptionMap()
  return optionKeys.some((key) => current[key] !== original[key])
}
...
const isValid = computed(() => {
  ...
  if (!isEssay.value) {
    ...
    if (selectedAnswersMissingText()) return false
  }
  return true
})

const buildQuestion = () => {
  const originalType = String(props.question?.type || 'SINGLE_CHOICE')
  const base = {
  ...
  const normalizedCorrectAnswers = [...new Set(
    localCorrectAnswers.value.map((value) => String(value).trim().toUpperCase()).filter(Boolean)
  )]
  const optionRows = optionKeys
    .map((key) => ({
      id: key,
      text: localOptions[key].trim(),
      isCorrect: normalizedCorrectAnswers.includes(key)
    }))
    .filter((row) => row.text)

  const originalContent = String(props.question?.content ?? '').trim()
  const contentChanged = localContent.value.trim() !== originalContent
  const typeChanged = localType.value !== originalType
  const optionsChanged = haveOptionsChanged()
  const shouldResetStemDerived = contentChanged || typeChanged
  const shouldResetRender = shouldResetStemDerived || optionsChanged
  const shouldResetOptionDerived = optionsChanged || typeChanged
  const shouldResetParseIssues = shouldResetStemDerived || shouldResetOptionDerived

  if (isMultiple.value) {
    base.correctAnswer = normalizedCorrectAnswers
    base.correctOption = normalizedCorrectAnswers[0] || ''
    base.options = optionRows
  } else if (isEssay.value) {
    base.correctAnswer = originalType === 'ESSAY'
    ...
    base.correctAnswer = normalizedCorrectAnswers[0] || ''
    base.correctOption = normalizedCorrectAnswers[0] || ''
    base.options = optionRows
  }
  ...
  if (shouldResetStemDerived) {
    base.latexContent = undefined
    base.contentType = undefined
  }
  if (shouldResetRender) {
    base.render = null
    base.renderMode = null
    base.render_mode = null
  }
  if (shouldResetOptionDerived) {
    base.latexOptions = undefined
  }
  if (shouldResetParseIssues && Array.isArray(props.question?.issues)) {
    base.issues = []
  }
  if (shouldResetParseIssues) {
    base.parseConfidence = null
  }
...
const handleSave = () => {
  ...
  if (!isEssay.value) {
    ...
    if (selectedAnswersMissingText()) {
      errors.options = 'Đáp án đúng phải có nội dung'
      return
    }
  }
...
const resetForm = () => {
  if (props.question) {
    ...
    localOptions.A = optionTextFromQuestion(props.question, 'A')
    localOptions.B = optionTextFromQuestion(props.question, 'B')
    localOptions.C = optionTextFromQuestion(props.question, 'C')
    localOptions.D = optionTextFromQuestion(props.question, 'D')
    const ca = props.question.type === 'ESSAY'
      ? []
      : (props.question.correctAnswer ?? props.question.correctOption ?? [])
    localCorrectAnswers.value = normalizeCorrectAnswers(ca)
...
/* Transition */
.qemodal-enter-active, .qemodal-leave-active { transition: opacity 0.2s ease; }
.qemodal-enter-from, .qemodal-leave-to { opacity: 0; }
.qemodal-enter-from .qemodal, .qemodal-leave-to .qemodal { opacity: 0; }
</style>