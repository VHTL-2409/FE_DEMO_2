<template>
  <div class="tui-page-wrap">
    ...
    <!-- Error Alert -->
    <div v-if="errorMsg" class="tui-alert tui-alert--danger admin-dashboard__alert">
      <LucideIcon name="alert_circle" :size="16" />
      <span>{{ errorMsg }}</span>
      <button type="button" aria-label="Dong thong bao loi" @click="errorMsg = ''"><LucideIcon name="x" :size="14" /></button>
    </div>

    <!-- Stat Strip — KPI cards with functional icons -->
    <div class="tui-stat-grid admin-dashboard__stats">
      <div
    ...
    <div ref="contentScrollEl" class="admin-dashboard__content">

      <!-- Row 1: So sanh KPI (bar ngang) + Radar tong quan -->
      <div class="admin-dashboard__grid admin-dashboard__grid--top">

        <div class="tui-panel tui-panel--anim admin-dashboard__panel">
          ...
          <div class="admin-dashboard__chart admin-dashboard__chart--large">
            <div ref="kpiHBarChartRef" class="admin-dashboard__chart-canvas admin-dashboard__chart-canvas--large" />
            <div v-if="loading && !stats" class="tui-skeleton admin-dashboard__chart-skeleton" />
          </div>
        </div>
        ...
        <div class="tui-panel tui-panel--anim admin-dashboard__panel">
          ...
          <div class="admin-dashboard__chart admin-dashboard__chart--large">
            <div ref="radarChartRef" class="admin-dashboard__chart-canvas admin-dashboard__chart-canvas--large" />
            <div v-if="loading && !stats" class="tui-skeleton admin-dashboard__chart-skeleton" />
          </div>
        </div>
      </div>
      ...
      <div class="admin-dashboard__grid admin-dashboard__grid--bottom">
        ...
        <div class="tui-panel tui-panel--anim admin-dashboard__panel">
          ...
          <div class="admin-dashboard__chart">
            <div ref="pieChartRef" class="admin-dashboard__chart-canvas" />
            <div v-if="loading && !stats" class="tui-skeleton admin-dashboard__chart-skeleton" />
          </div>
          <div class="admin-dashboard__legend">
            <div v-for="(item, i) in userDistLegend" :key="i" class="tui-metric-row admin-dashboard__legend-row">
              <div class="tui-metric-label-row">
                <span class="tui-metric-label admin-dashboard__legend-label">
                  <span class="admin-dashboard__legend-swatch" :style="{ background: item.color }" />
                  {{ item.label }}
                </span>
        ...
        <div class="tui-panel tui-panel--anim admin-dashboard__panel">
          ...
          <div class="admin-dashboard__chart">
            <div ref="barChartRef" class="admin-dashboard__chart-canvas" />
            <div v-if="loading && !stats" class="tui-skeleton admin-dashboard__chart-skeleton" />
          </div>
        </div>
        ...
        <div class="tui-panel tui-panel--anim admin-dashboard__panel">
          ...
          <div class="admin-dashboard__chart">
            <div ref="lineChartRef" class="admin-dashboard__chart-canvas" />
            <div v-if="loading && !stats" class="tui-skeleton admin-dashboard__chart-skeleton" />
          </div>
        </div>
      </div>
...
.admin-dashboard__grid--top {
  grid-template-columns: repeat(2, minmax(0, 1fr));
}

.admin-dashboard__grid--bottom {
  grid-template-columns: minmax(240px, 280px) repeat(2, minmax(0, 1fr));
}
...
.admin-dashboard__chart--large {
  min-height: 240px;
}

.admin-dashboard__chart-canvas {
  width: 100%;
  height: 100%;
  min-height: 200px;
}

.admin-dashboard__chart-canvas--large {
  min-height: 220px;
}

.admin-dashboard__chart-skeleton {
  position: absolute;
  inset: 0.75rem;
  border-radius: 12px;
}
...
.admin-dashboard__legend-swatch {
  width: 8px;
  height: 8px;
  border-radius: 2px;
