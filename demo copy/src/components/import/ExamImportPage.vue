      ...
      <div class="eip__main-col">
        ...
        <div v-if="!activeSessionId" class="eip__quick-actions">
          <button type="button" class="eip__action-card-btn" @click="goToImport">
            ...
            <span class="eip__action-card-title">Mở luồng import đầy đủ</span>
            <span class="eip__action-card-sub">Chuyển sang màn tạo đề để nhập từ nhiều nguồn</span>
          </button>
        ...
        <div v-if="activeSessionId && parsedQuestions.length === 0 && sessionStatus === 'DONE'" class="eip__no-questions">
          ...
          <p>Không tìm thấy câu hỏi đọc được trong file.</p>
          <p class="eip__no-questions-sub">{{ noQuestionsMessage }}</p>
        </div>
      </div>
      ...
      <div class="eip__side-col">
        ...
        <div v-if="parsedQuestions.length > 0" class="eip__exam-select-card">
          ...
          <select v-model="selectedExamId" class="eip__exam-select">
            <option value="">Tạo bài kiểm tra mới từ file này</option>
          ...
          <p v-if="!selectedExamId" class="eip__exam-hint">
            Nếu không chọn đề đích, hệ thống sẽ tự tạo một bài kiểm tra hoàn chỉnh rồi gắn toàn bộ câu hỏi vào đó.
          </p>
        </div>
...
const noQuestionsMessage = computed(() => {
  const warnings = report.value?.warnings
  if (Array.isArray(warnings) && warnings.length > 0) {
    return warnings[0]
  }
  return 'File có thể là ảnh quét, nội dung quá nhiễu hoặc parser hiện chưa sẵn sàng. Thử lại sau hoặc dùng file rõ nét hơn.'
})
...
async function pollSession() {
  ...
  try {
    ...
    } else if (preview?.questions?.length === 0 && Array.isArray(preview?.report?.warnings) && preview.report.warnings.length > 0) {
      toast.error(preview.report.warnings[0])
    }
  } catch (err) {
...
async function handleConfirm() {
  ...
  try {
    ...
    const targetExamId = result?.examId || selectedExamId.value || ''
    toast.success(
      result?.createdExam
        ? `Đã tạo bài kiểm tra mới và lưu ${importedCount} câu hỏi!`
        : `Đã lưu ${importedCount} câu hỏi vào bài kiểm tra!`
    )
    if (importedCount > 0 && targetExamId) {
      await router.push(`/teacher/exams/${targetExamId}`)
    }
    resetSession()
  } catch (err) {
