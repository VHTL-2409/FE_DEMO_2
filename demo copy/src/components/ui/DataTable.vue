<template>
  <div
    class="ds-table-wrapper relative overflow-hidden rounded-[var(--ds-radius-lg)] border border-[var(--ds-border)] bg-[var(--ds-surface)] shadow-[var(--ds-shadow-sm)]"
    :aria-busy="loading ? 'true' : undefined"
  >
    <!-- Loading overlay -->
    <div
      v-if="loading"
      class="absolute inset-0 z-10 flex items-center justify-center bg-[var(--ds-surface)]/92"
      role="status"
      aria-live="polite"
    >
...
const toggleSelectAll = () => {
  ...
  else selectedRows.value = props.data.map((row, i) => (props.rowKey ? getRowId(row) : i))
  emit('selection-change', selectedRows.value)
}
</script>
