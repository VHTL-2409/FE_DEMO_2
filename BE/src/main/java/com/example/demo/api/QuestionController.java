...
public class QuestionController {
    ...
    public ApiResponse<List<QuestionResponse>> list(@PathVariable Long examId,
        ...
        boolean shuffleEnabled = Boolean.TRUE.equals(exam.getShuffleQuestions()) || Boolean.TRUE.equals(exam.getShuffleAnswers());
        if (!includeCorrectAnswer && shuffleEnabled && attemptId == null) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "attemptId is required for shuffled exam questions");
        }
        return ApiResponse.success(questionService.listByExam(exam, includeCorrectAnswer, actor, attemptId));
    }
