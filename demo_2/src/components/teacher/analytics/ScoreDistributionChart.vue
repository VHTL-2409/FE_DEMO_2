<template>
  <div class="sdc">
    ...
    <div class="sdc__chart-wrap" ref="chartEl">
      ...
      <!-- ECharts -->
      <canvas v-show="!loading && hasData" ref="canvasEl" class="sdc__canvas" />
    </div>
...
const initChart = async () => {
  ...
  chartInstance = ec.init(canvasEl.value, null, { renderer: 'canvas' })
  ...
  return {
    ...
    series: [{
      ...
      lineStyle: { 
        color: '#b15f00', 
        width: 3,
        shadowColor: 'rgba(139, 75, 0, 0.25)',
        shadowBlur: 8
      },
      label: {
        show: values.value.some(v => v > 0),
...
watch([hasData, chartType], async ([data, type]) => {
  if (data) {
    await nextTick()
    await initChart()
  }
