<template>
  <div :class="isDark ? 'dark' : 'light'" class="flex h-full min-h-0 flex-1 flex-col bg-[#faf9f5] font-display text-slate-900 dark:bg-slate-950 dark:text-slate-100">
    <div class="relative flex min-h-0 w-full flex-1 flex-col overflow-x-hidden overflow-y-hidden">
      <div class="layout-container flex min-h-0 flex-1 grow flex-col">
        ...
        <main class="teacher-stitch-main teacher-page-shell portal-scrollbar relative mx-auto flex w-full max-w-none min-h-0 flex-1 flex-col overflow-x-hidden overflow-y-auto">
          <div class="pointer-events-none absolute -top-16 -left-16 size-72 rounded-full bg-primary/12 blur-3xl animate-float-slow"></div>
          ...
          <nav class="relative mb-6 flex flex-wrap items-center gap-2 animate-fade-up text-xs font-medium text-slate-500">
            ...
            <RouterLink to="/teacher/live-monitoring" class="transition hover:text-[var(--role-primary)]">Giám sát</RouterLink>
            <span class="material-symbols-outlined text-[14px] text-slate-400">chevron_right</span>
            <span class="max-w-[min(100%,16rem)] break-words leading-5" :title="examName">{{ examName }}</span>
            <span class="material-symbols-outlined text-[14px] text-slate-400">chevron_right</span>
            <span class="max-w-[min(100%,18rem)] break-words font-semibold leading-5 text-[var(--role-primary)]" :title="studentName">
              {{ studentName }}
            </span>
          </nav>
          ...
          <div class="glass-card rounded-2xl p-6 mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-lg animate-fade-up-delay">
            <div class="flex gap-5 items-center">
              ...
              <div>
                <div class="flex items-center gap-3 mb-1 flex-wrap">
                  <h1 class="stitch-font-headline text-3xl font-bold tracking-tight text-amber-900 dark:text-amber-100 md:text-4xl">{{ studentName }}</h1>
            ...
            <div class="w-full md:w-auto">
              <button class="w-full md:w-auto flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-primary hover:bg-primary-700 text-white font-semibold text-sm transition-all shadow-lg shadow-primary/25" type="button" @click="openLivestreamModal">
            ...
            <div class="lg:col-span-2 flex flex-col gap-4 sm:gap-6">
              <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                <div class="glass-card rounded-2xl p-5 shadow-lg">
                  ...
                  <span class="text-primary dark:text-amber-300 text-xs font-bold mt-2 inline-block">{{ progressPercent }}% hoàn thành</span>
                </div>
              ...
              <div class="modal-header bg-slate-50/50 dark:bg-slate-800/30">
                <div class="flex items-center gap-3">
                  <div class="size-10 rounded-xl bg-primary/10 dark:bg-primary/20 flex items-center justify-center">
                    <span class="material-symbols-outlined text-primary dark:text-amber-300 text-xl">live_tv</span>
                  </div>
                  <div>
                ...
                <div v-else>
                  ...
                  <div v-else class="space-y-4">
                    <div v-for="(question, idx) in liveExamQuestions" :key="question.id" class="rounded-2xl glass-card p-5 border border-slate-200/50 dark:border-slate-700/50 hover:shadow-lg transition-shadow">
                      <p class="text-xs font-bold text-primary dark:text-amber-300 mb-2">Câu {{ idx + 1 }}</p>
                      <p class="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-4">{{ question.content }}</p>
                      <div class="space-y-2">
                        <div
                          v-for="option in question.options"
                          :key="`${question.id}-${option.id}`"
                          :class="liveAnswersByQuestionId[question.id] === option.id ? 'border-primary bg-primary/5 dark:bg-primary/15 text-primary dark:text-amber-200 ring-1 ring-primary/30' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'"
