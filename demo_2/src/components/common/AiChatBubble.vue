<template>
  <div v-if="isTeacher" class="fixed bottom-4 right-4 z-[100] flex flex-col items-end gap-2 font-display">
    <transition name="fade-scale">
      <div
        v-show="open"
        class="w-[min(100vw-2rem,22rem)] h-[min(70vh,28rem)] flex flex-col rounded-2xl shadow-2xl border border-slate-200/80 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md overflow-hidden"
        role="dialog"
...
const send = async () => {
  ...
  try {
    ...
    if (data?.status === 'ERROR' || data?.error) {
      toastService.warn(data?.error ? String(data.error) : reply || 'Không nhận được phản hồi.')
    }
    messages.value.push({ role: 'assistant', content: reply || '—' })
  } catch (e) {
    toastService.error(e?.message || 'Không gửi được tin nhắn.')
    messages.value.push({
      role: 'assistant',
