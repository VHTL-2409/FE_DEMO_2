  ...
  <div class="flex flex-col gap-2">
    <div v-if="label || $slots.label" class="flex items-center justify-between gap-2">
      <label
        ...
        :for="fieldId"
        class="text-[0.82rem] font-bold uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400"
      >
    ...
    <slot
      ...
      :error-id="errorId"
    />
    <p v-if="hint && !error" :id="hintId" class="text-xs leading-5 text-slate-500 dark:text-slate-400">
      {{ hint }}
    </p>
