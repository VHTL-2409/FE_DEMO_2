  ...
  switch (alert.type) {
    case 'RISK_UPDATED':
      addAlert({
        ...
        reviewRequired: Boolean(alert.reviewRequired),
        recommendedAction: alert.recommendedAction || '',
        reasons: alert.reasons || [],
        evidenceSummary: alert.evidenceSummary || [],
        severity: alert.riskScore >= 80 ? 'HIGH' : alert.riskScore >= 30 ? 'MEDIUM' : 'LOW',
        message: alert.message || `${alert.recommendedAction || 'Rủi ro'}: ${alert.riskScore ?? 0}%`,
      ...
      addEvent({
        type: 'ALERT',
        message: [
          alert.recommendedAction || `Rủi ro thay đổi → ${alert.riskScore ?? 0}%`,
          ...(alert.reasons || []).slice(0, 1)
        ].filter(Boolean).join(' • '),
...
const handleActionConfirm = async (reason) => {
  ...
  try {
    if (currentActionType === 'warn') {
      ...
      await pauseAttempt(actionTarget.value.attemptId, reason)
...
const handleBatchPause = async (ids) => {
  ...
  actionLoading.value = true
  try {
    await Promise.allSettled(ids.map(id => pauseAttempt(id, 'Tạm dừng hàng loạt')))
