...
const steps = [
  ...
  { id: 'review', label: 'Chỉnh sửa câu hỏi' },
...
onMounted(async () => {
  ...
  if (route.query.classId) {
    form.classId = String(route.query.classId)
    form.examType = 'private'
  }
})
...
const nextStep = () => {
  const idx = steps.findIndex(s => s.id === activeStep.value)
  if (activeStep.value === 'import' && !isQuestionsValid.value) {
    toast.error('Vui lòng đọc file và tạo ít nhất 1 câu hỏi trước khi sang bước chỉnh sửa.')
    return
  }
...
const jumpToStep = (stepId) => {
  if (stepId === 'review' && !isQuestionsValid.value) {
    toast.error('Vui lòng đọc file và tạo ít nhất 1 câu hỏi trước khi sang bước chỉnh sửa.')
...
const normalizeCorrectAnswerTokens = (value) => {
...
const getQuestionOptionMap = (question) => {
  const optionMap = new Map()
  if (Array.isArray(question?.options) && question.options.length > 0) {
    question.options.forEach((option) => {
      const id = String(option?.id ?? '').trim().toUpperCase()
      const text = String(option?.text ?? option?.content ?? option?.label ?? '').trim()
      if (id && text) {
        optionMap.set(id, text)
      }
    })
    return optionMap
  }
  ...
  ;[
    ...
    if (normalizedText) {
      optionMap.set(id, normalizedText)
    }
  })
  return optionMap
}

const isChoiceQuestionType = (question) => {
  const type = String(question?.type || 'SINGLE_CHOICE').toUpperCase()
  return type === 'SINGLE_CHOICE' || type === 'MULTIPLE_CHOICE'
}

const isQuestionPublishReady = (question) => {
  ...
  if (String(rawRenderMode || '').toLowerCase() === 'image') {
    return false
  }
  if (!isChoiceQuestionType(question)) {
    if (type === 'ESSAY') {
      return true
    }
    const answer = String(getQuestionCorrectAnswer(question) ?? '').trim()
    return answer !== ''
  }

  const optionMap = getQuestionOptionMap(question)
  if (optionMap.size < 2) {
    return false
  }

  const answers = normalizeCorrectAnswerTokens(getQuestionCorrectAnswer(question))
  if (answers.length === 0) {
    return false
  }
  ...
  return answers.every((answer) => optionMap.has(answer))
}

const areQuestionsPublishReady = computed(() =>
  form.questions.length > 0 && form.questions.every((question) => isQuestionPublishReady(question))
)
...
const canPublish = computed(() =>
  isInfoValid.value &&
  isConfigValid.value &&
  areQuestionsPublishReady.value &&
  !isSubmitting.value
)
...
const handlePublish = async () => {
  if (!canPublish.value) {
    ...
    if (!areQuestionsPublishReady.value) {
      toast.error('Vui lòng hoàn thiện câu hỏi trước khi xuất bản: mỗi câu trắc nghiệm cần ít nhất 2 đáp án có nội dung, đáp án đúng hợp lệ, và không còn ở chế độ ảnh fallback.')
...
function buildExamPayload(isActive = false) {
  const className = form.examType === 'private'
    ...
    : null
  const classId = form.examType === 'private' && form.classId
    ? Number(form.classId)
    : null
  return {
