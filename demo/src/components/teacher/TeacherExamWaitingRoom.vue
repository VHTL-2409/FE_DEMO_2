    ...
    <div class="wr-container">
      ...
      <div class="wr-header">
        ...
        <div class="wr-header__stats">
          <div class="wr-stat">
            <span class="wr-stat__value">{{ waitingCount }}</span>
...
const normalizeStatusCode = (studentOrStatus) => {
  const raw = typeof studentOrStatus === 'object'
    ? (studentOrStatus?.statusCode || studentOrStatus?.status)
    : studentOrStatus
  const text = String(raw || '').toUpperCase()
  if (STATUS_LABELS[text]) return text
  if (raw === 'Đang chờ') return 'WAITING'
  if (raw === 'Đang thi') return 'IN_PROGRESS'
  if (raw === 'Tạm dừng') return 'PAUSED'
  if (raw === 'Đã nộp') return 'SUBMITTED'
  if (raw === 'Bị dừng' || raw === 'Đã dừng') return 'STOPPED'
  return 'WAITING'
}

const statusLabel = (statusCode) => STATUS_LABELS[normalizeStatusCode(statusCode)] || 'Đang chờ'

const waitingCount = computed(() =>
  waitingStudents.value.filter(s => normalizeStatusCode(s) === 'WAITING').length
)

const inProgressCount = computed(() =>
  waitingStudents.value.filter(s => normalizeStatusCode(s) === 'IN_PROGRESS').length
)

const submittedCount = computed(() =>
  waitingStudents.value.filter(s => ['SUBMITTED', 'AUTO_SUBMITTED'].includes(normalizeStatusCode(s))).length
)
  ...
  switch (event.type) {
    ...
    case 'STUDENT_JOINED':
    {
      ...
      upsertStudent({
        ...
        joinedAt: event.joinedAt || event.issuedAt || new Date().toISOString()
      })
      addFlashEvent(`${displayName} đã vào phòng chờ`)
      break
    }
    ...
    case 'ATTEMPT_JOINED': {
      ...
      upsertStudent({
        ...
        riskScore: event.riskScore ?? 0,
        joinedAt: event.joinedAt || event.startedAt || event.issuedAt || new Date().toISOString()
      })
      updateStudentStatus(event.attemptId, 'IN_PROGRESS')
    ...
    case 'EXAM_SUBMITTED':
      {
        const nextStudent = {
          ...
          studentEmail: event.studentEmail || event.email || event.student?.email || '—',
          status: statusLabel(event.type === 'AUTO_SUBMITTED' ? 'AUTO_SUBMITTED' : 'SUBMITTED'),
          statusCode: event.type === 'AUTO_SUBMITTED' ? 'AUTO_SUBMITTED' : 'SUBMITTED',
      ...
      updateStudentStatus(event.attemptId, event.type === 'AUTO_SUBMITTED' ? 'AUTO_SUBMITTED' : 'SUBMITTED')
      addFlashEvent(`${event.studentName || 'Học sinh'} đã nộp bài`)
      break

    case 'ATTEMPT_PAUSED':
    case 'STUDENT_PAUSED':
      updateStudentStatus(event.attemptId, 'PAUSED')
    ...
    case 'STUDENT_STOPPED':
      updateStudentStatus(event.attemptId, 'STOPPED')
...
function upsertStudent(student) {
  const normalized = {
    ...student,
    statusCode: normalizeStatusCode(student),
    status: statusLabel(student)
  }
  const existing = waitingStudents.value.findIndex(s => s.attemptId === normalized.attemptId)
  if (existing >= 0) {
    waitingStudents.value[existing] = { ...waitingStudents.value[existing], ...normalized }
  } else {
    waitingStudents.value = [...waitingStudents.value, normalized]
  }
}
...
function updateStudentStatus(attemptId, status) {
  ...
  if (idx >= 0) {
    ...
    waitingStudents.value[idx] = { ...waitingStudents.value[idx], statusCode, status: statusLabel(statusCode) }
  }
}
...
const statusRank = (studentOrStatus) => {
  const code = normalizeStatusCode(studentOrStatus)
  if (code === 'SUBMITTED' || code === 'AUTO_SUBMITTED' || code === 'STOPPED') return 3
  if (code === 'IN_PROGRESS' || code === 'PAUSED') return 2
  return 1
}

const normalizeStudent = (student) => {
  const statusCode = normalizeStatusCode(student)
  return {
    ...student,
    statusCode,
...
const mergeFetchedStudents = (students) => {
  const existingByAttempt = new Map(waitingStudents.value.map((student) => [student.attemptId, student]))
  return (students || []).map((student) => {
    const normalized = normalizeStudent(student)
    const existing = existingByAttempt.get(normalized.attemptId)
    if (existing && statusRank(existing) > statusRank(normalized)) {
      return existing
    }
    return normalized
  })
...
const fetchWaitingStudents = async () => {
  ...
  try {
    ...
    waitingStudents.value = mergeFetchedStudents(students)
    lastSyncedAt.value = Date.now()
  } catch (err) {
...
const getStudentCardClass = (student) => {
  const code = normalizeStatusCode(student)
  if (code === 'IN_PROGRESS') return 'wr-student-card--active'
  if (code === 'SUBMITTED' || code === 'AUTO_SUBMITTED') return 'wr-student-card--submitted'
  if (student.riskScore > 50) return 'wr-student-card--warning'
  return ''
}

const getStatusBadgeClass = (studentOrStatus) => {
  const code = normalizeStatusCode(studentOrStatus)
  if (code === 'IN_PROGRESS') return 'wr-status-badge--active'
  if (code === 'SUBMITTED' || code === 'AUTO_SUBMITTED') return 'wr-status-badge--submitted'
  if (code === 'PAUSED') return 'wr-status-badge--paused'
