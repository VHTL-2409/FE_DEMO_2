<template>
  <header class="sticky top-0 z-50 w-full bg-[#fcf8ff]/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-[#e4e1ed]/90 dark:border-slate-800/80 shadow-sm supports-[backdrop-filter]:bg-[#fcf8ff]/82 supports-[backdrop-filter]:dark:bg-slate-900/85">
    <div class="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center h-16">
        <div class="flex items-center gap-6 lg:gap-10">
          <div class="flex items-center gap-3">
            <div class="size-9 rounded-xl bg-[#5d5fef] flex items-center justify-center text-white shadow-lg shadow-[#5d5fef]/30">
              <span class="material-symbols-outlined text-lg">school</span>
            </div>
            <div class="flex flex-col">
              <span class="text-[#5d5fef] dark:text-indigo-300 text-lg font-extrabold leading-tight tracking-tight font-[family-name:Manrope]">EduExam</span>
              <span class="text-slate-500 dark:text-slate-400 text-[10px] font-bold uppercase tracking-[0.2em]">Hệ thống quản lý thi</span>
            </div>
          </div>
          <nav class="hidden md:flex items-center gap-1">
            <RouterLink :class="navClass('dashboard')" to="/teacher/dashboard" class="px-3.5 py-2 text-sm font-semibold transition-all duration-200">
              Trang chủ
            </RouterLink>
            <RouterLink :class="navClass('exam')" to="/teacher/exams" class="px-3.5 py-2 text-sm font-semibold transition-all duration-200">
              Tạo đề thi
            </RouterLink>
            <RouterLink :class="navClass('exam-list')" to="/teacher/exams/list" class="px-3.5 py-2 text-sm font-semibold transition-all duration-200">
              Danh sách đề thi
            </RouterLink>
            <RouterLink :class="navClass('monitoring')" to="/teacher/live-monitoring" class="px-3.5 py-2 text-sm font-semibold transition-all duration-200">
              Giám sát
            </RouterLink>
            <RouterLink :class="navClass('profile')" to="/teacher/profile" class="px-3.5 py-2 text-sm font-semibold transition-all duration-200">
              Hồ sơ
            </RouterLink>
          </nav>
        </div>
...
const navClass = (section) => {
  if (props.activeSection === section) {
    return 'text-[#5d5fef] bg-white dark:bg-slate-800 ring-1 ring-[#5d5fef]/20 shadow-sm rounded-xl'
  }
  return 'text-slate-600 dark:text-slate-400 hover:text-[#5d5fef] hover:bg-[#f5f2fe] dark:hover:bg-slate-800 rounded-xl'
}
