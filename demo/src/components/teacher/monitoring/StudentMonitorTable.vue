    ...
    <div class="smt__table-wrapper">
        ...
        <tbody>
          <template v-for="student in sortedStudents" :key="student.id || student.attemptId">
            ...
            >
              ...
              <td class="smt__td">
                <div class="smt__student">
                  ...
                  <div class="smt__avatar" :class="`smt__avatar--${statusColor(student)}`">
                    ...
                    <span v-else-if="student.student">{{ student.student.charAt(0).toUpperCase() }}</span>
                    <LucideIcon name="person" v-else />
                  </div>
                  <div class="smt__student-info">
                    <p class="smt__student-name">{{ student.userName || student.student || student.fullName || student.email || '—' }}</p>
              ...
              <!-- Status -->
              <td class="smt__td">
                <div class="smt__review">
                  <span
                    class="smt__review-chip"
                  ...
                  <p v-if="student.reasons?.length" class="smt__review-reason">
                    {{ student.reasons[0] }}
                  </p>
                </div>
              </td>
            ...
            >
              <td colspan="8">
                <div class="smt__expand-content">
                  ...
                  <div v-if="student.evidenceSummary?.length" class="smt__expand-section">
                    <h5 class="smt__expand-title">Bằng chứng nổi bật</h5>
                    <div class="smt__evidence-list">
                      <span
                        v-for="item in student.evidenceSummary"
                        :key="item"
                        class="smt__evidence-chip"
                      >
                        {{ item }}
                      </span>
                    </div>
      ...
      <div v-if="sortedStudents.length === 0" class="smt__empty">
        ...
        <p class="smt__empty-title">
          {{ hasSearch ? 'Không tìm thấy học sinh nào' : 'Chưa có học sinh nào' }}
        </p>
        <p class="smt__empty-desc">
          {{ hasSearch ? 'Thử thay đổi bộ lọc hoặc từ khóa tìm kiếm' : 'Học sinh sẽ xuất hiện khi bắt đầu thi' }}
        </p>
      </div>
    </div>
  </div>
...
const { clampScore, resolveLevel } = useRiskLevel()
...
const sortedStudents = computed(() => {
  ...
  switch (props.sortBy) {
    ...
    case 'name':
      return arr.sort((a, b) => (a.userName || a.student || '').localeCompare(b.userName || b.student || ''))
    case 'violations':
...
const statusColor = (student) => {
  ...
  if (raw === 'IN_PROGRESS' || raw === 'ACTIVE') return 'online'
...
.dark .smt__batch-btn { background: var(--ds-gray-800); border-color: var(--ds-border-strong); color: var(--ds-text-secondary); }
...
.dark .smt__batch-btn--danger { color: var(--ds-danger); }
...
.smt__row--high_risk { border-left-color: var(--ds-warning); }
.smt__row--suspicious { border-left-color: var(--ds-accent); }
...
.smt__avatar--paused { background: var(--ds-warning-soft); color: var(--ds-warning); }
...
.dark .smt__student-name { color: var(--ds-text); }
...
.dark .smt__score { color: var(--ds-text); }
...
.smt__violation-badge--medium { background: var(--ds-warning-soft); color: var(--ds-warning); }
...
.smt__risk-fill--suspicious { background: var(--ds-accent); }
.smt__risk-fill--high_risk { background: var(--ds-warning); }
...
.smt__risk-val--suspicious { color: var(--ds-warning); }
...
.smt__status-chip--paused { background: var(--ds-warning-soft); color: var(--ds-warning); }
...
.dark .smt__action-btn--danger:hover { background: var(--ds-danger-soft); }
...
.dark .smt__expand-title { color: var(--ds-text); }
...
.dark .smt__expand-item > span:last-child { color: var(--ds-text); }
...
.dark .smt__empty-title { color: var(--ds-text); }
