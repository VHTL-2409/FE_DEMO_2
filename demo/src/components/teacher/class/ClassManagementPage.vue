    ...
    <div class="cmp__toolbar">
      <div class="cmp__search">
        ...
        <input
          ...
          @input="currentPage = 1"
        />
        <button v-if="searchQuery" type="button" class="cmp__search-clear" @click="searchQuery = ''">
          <LucideIcon name="x" />
        </button>
      </div>
      ...
      <TransitionGroup name="card">
        ...
        >
          ...
          <div class="cmp__card-header">
            <div class="cmp__card-icon">
              <LucideIcon name="groups" />
            </div>
            <div class="cmp__card-actions" @click.stop>
          ...
          <div class="cmp__card-body">
            ...
            <div v-if="cls.subject" class="cmp__card-subject">
              <LucideIcon name="book" />
              <span>{{ cls.subject }}</span>
            </div>
            <p v-if="cls.description" class="cmp__card-desc">{{ cls.description }}</p>
          </div>

          <!-- Card Footer -->
          <div class="cmp__card-footer">
            <div class="cmp__card-stat">
              <LucideIcon name="users" />
              <span>{{ cls.studentCount || 0 }} học sinh</span>
            </div>
            <button type="button" class="cmp__card-manage-btn" @click.stop="openClassDetail(cls)">
              <span>Quản lý</span>
      ...
      <div v-if="showSuccessModal" class="cmp__modal-overlay" @click.self="showSuccessModal = false">
        <div class="cmp__modal cmp__modal--success">
          ...
          <div class="cmp__class-code-box">
            ...
            <button type="button" class="cmp__copy-btn" @click="copyClassCode" title="Sao chép mã">
...
const handleClassCreated = (classData) => {
  if (classData && classData.classCode) {
    createdClassCode.value = classData.classCode
    showSuccessModal.value = true
    copied.value = false
  }
}
