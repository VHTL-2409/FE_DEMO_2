    ...
    <div class="login-container">
      ...
      <div class="login-form-panel">
        <div class="login-form-wrap">
          ...
          <!-- Google OAuth button -->
          <button type="button" class="login-google-btn" @click="loginWithGoogle">
...
// Check for errors from OAuth2 callback redirect
onMounted(() => {
  const params = new URLSearchParams(window.location.search)
  const error = params.get('error')
  if (error === 'google_denied') {
    toast.warning('Đăng nhập Google đã bị hủy.')
  } else if (error === 'google_oauth_failed') {
    toast.error('Đăng nhập Google thất bại. Vui lòng thử lại.')
  } else if (error === 'google_not_configured') {
    toast.error('Đăng nhập Google chưa được cấu hình. Vui lòng liên hệ quản trị viên.')
  }
})

const loginWithGoogle = () => {
  let apiBase = (import.meta.env.VITE_API_BASE_URL || 'http://localhost:8082').replace(/\/$/, '')
  // Nếu nhầm trỏ VITE_API_BASE_URL sang cổng Vite, OAuth init phải gọi backend :8082
  if (import.meta.env.DEV && /:(5173|4173)(\/|$)/.test(apiBase)) {
    console.warn('[EduExam] VITE_API_BASE_URL đang trỏ cổng frontend; chuyển sang http://localhost:8082 cho Google OAuth.')
    apiBase = 'http://localhost:8082'
  }
  window.location.href = `${apiBase}/api/auth/oauth2/google/init`
}
...
const onSubmit = async () => {
  ...
  try {
    ...
    toast.success(`Chào mừng ${authData?.username || 'bạn'}! Đăng nhập thành công.`)
